...
void TestPipePrintFunc(const wchar_t* strPtr)
{
...
PYTHON_CALLABLE bool LoadIntoMainProcess()
{
  ...
  g_sublimePipe = CreateFileW(pipeName.c_str(), GENERIC_WRITE | GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
  if (!remoteThread)
  {
...
PYTHON_CALLABLE int32_t FindTopLevelWindows()
{
  if (g_sublimeProcess == INVALID_HANDLE_VALUE)
  {
    g_PrintFunc(L"Not loaded");
